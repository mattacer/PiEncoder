...
/*
	Note: This code was originally authored by David H. Bailey in C (available at 
	http://www.experimentalmath.info/bbp-codes) and modified for use with PiEncoder.
	TODO: Remove unneeded functions, tidy up, and use more digits of pi per term(currently 1 to 2 per).
*/

#include <math.h>
#include "piqpr8.h"

void ihex(double x, int ind, char* chx)
...
void PiDigits(int index, int numberOfDigits, char* dig)
{
...
#define NHX 16
	char chx[NHX];
	dig[numberOfDigits - 1] = '\0';
	//for (id = 0; id < NHX; id++){
	/*  id is the digit position.  Digits generated follow immediately after id. */
	//for (int i = 0; i < numberOfDigits; i++){
	for (int i = 0; dig[numberOfDigits-1] == '\0' ; i++){
		s1 = series(1, id + i);
		s2 = series(4, id + i);
		s3 = series(5, id + i);
		s4 = series(6, id + i);
